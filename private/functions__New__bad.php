...
function verifyMilton($city) {
  $enteredCity = strtoupper($city);
  if($enteredCity == "ELGIN")
  {
    lb();
    echo "Patron Entered Elgin";
    lb();
  }
  else {
    echo "Patron didn't enter elgin";
  }
}
...
function getWard($address) {
  ...
  if($pointLocation->pointInPolygon($sampleCoords, burlington) == 'inside') {
    ...
    return "Coords are out of Gail Borden Wards";
  }

}

function isPatronInsideGBCatchmentArea($address) {
  $latlong = getLatLong($address);
  $latitude = $latlong[0];
  $longitude = $latlong[1];
  $sampleCoords = $latitude . ', ' . $longitude;
  $pointLocation = new pointLocation();
  if($pointLocation->pointInPolygon($sampleCoords, catchment1) == 'inside') {
    return true;
  }
  if($pointLocation->pointInPolygon($sampleCoords, catchment2) == 'inside') {
    return true;
  }
  if($pointLocation->pointInPolygon($sampleCoords, catchment3) == 'inside') {
    return true;
  }
  if($pointLocation->pointInPolygon($sampleCoords, catchment4) == 'inside') {
    return true;
  }
}
...
function getPatronWardByID($patronID) {
  ...
  if(!in_array($wardResult, array("WARD 1", "WARD 2", "WARD 3", "WARD 4"))) {
    echo "Patron is outside of Gail Borden";
  }
}

function isPatronLocal($patronID) {
  $patronWard = getPatronWardByID($patronID);
  if($patronWard == 'Patron is outside of Gail Borden') {
    echo 'outsider';
  }
}
...
//  $randomPIN = generateRandomPIN(0,9,minimumPinLength);
  ...
  $yearFromToday = strtotime('+1 year');
  $expirationString = date('Y-m-d', $yearFromToday);
  $homelib = $patronArrayObj['homelib'];
  $email = $patronArrayObj['email'];
  $name = $patronArrayObj['name'];
  $addressStreet = $patronArrayObj['addressStreet'];
  $citycommaProvince = $patronArrayObj['$citycommaProvince'];
  ...
  $phoneNumber = $patronArrayObj['phonenumber'];
  $numberType = $patronArrayObj['numberType'];
  $phoneNumber2 = $patronArrayObj['phonenumber2'];
  $numberType2 = $patronArrayObj['numberType2'];
  $randomPIN = $patronArrayObj['pin'];
  //$pin = substr($phoneNumber, -4,4);
  //$barcode = $patronArrayObj['barcode'];
  //$barcode = '21387001717764';  TEST CARD USED SO I DONT CHEW UP EXTRA CARDS IN MY CARD DB
  $expirationDate = $expirationString;
  ...
  $query_string ='{';
  $query_string .='"homeLibraryCode": "';
  $query_string .=strip_tags($homelib);
  $query_string .='","emails": ["';
  $query_string .=strip_tags($email);
  $query_string .='"],"names": ["';
  $query_string .=strip_tags($name);
  ...
  $query_string .= '"},{"number": "';
  $query_string .= strip_tags($phoneNumber2);
  $query_string .= '","type": "';
  $query_string .= strip_tags($numberType2);

  $query_string .= '"}],"pin": "';
  $query_string .= strip_tags($randomPIN);
  $query_string .= '","barcodes": ["';
...
function getOnlineRegistrations() {
  ...
  $query_string = '{
    ...
    "expr": {
      "op": "has",
      "operands": [
        "Created via GBPL OnlinePatronCreationForm v1",
        ""
      ]
    }
  }';
